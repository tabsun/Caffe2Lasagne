...
class CaffeMaxPool2DLayer(layers.MaxPool2DLayer):
    def __init__(self, incoming, pool_size, stride=None, ignore_border=True, **kwargs):
        if stride is None:
            self.stride=pool_size
        else:
            self.stride=stride
        super(CaffeMaxPool2DLayer,self).__init__(incoming,pool_size,stride,ignore_border=ignore_border,**kwargs)

    def get_output_shape_for(self, input_shape):
        output_shape = list(input_shape)  # copy / convert to mutable list
        output_shape[2] = (output_shape[2] - self.pool_size[0])/self.stride[0] + 1
        output_shape[3] = (output_shape[3] - self.pool_size[1])/self.stride[1] + 1
        return tuple(output_shape)

    def get_output_for(self, input, *args, **kwargs):
        return downsample.max_pool_2d(input, self.pool_size, st=self.stride, ignore_border=self.ignore_border)

class CaffeConv2DLayer(layers.Conv2DLayer):
    def __init__(self, incoming, num_filters, filter_size, group=1, stride=(1, 1), border_mode="valid", untie_biases=False, W=init.Uniform(), b=init.Constant(0.), nonlinearity=nonlinearities.rectify,convolution=T.nnet.conv2d, **kwargs):
        self.group= group
        # actually border_mode is useless in lasagne so I removed it
        super(CaffeConv2DLayer,self).__init__(incoming, num_filters, filter_size, stride=stride, pad=border_mode,untie_biases=untie_biases, W=W, b=b, nonlinearity=nonlinearity,convolution=convolution, **kwargs)
    ...
    def get_output_for(self, input, input_shape=None, *args, **kwargs):
        ...
        if self.border_mode in ['valid', 'full']:
            ...
            for g in range(self.group):
                ...
                tensors.append(self.convolution(inp, self.W[g*(self.num_filters/2):(g+1)*(self.num_filters/2),:,:,:], subsample=self.stride,
